...
load('yeo_RS7_Schaefer400S.mat'); % Yeo Ordering of the matrices (LR hemispheres together)  
...
if flags.TimeScale1==1
    ...
    for t=1:n_wsize
        ...
        Good_ICC = ICC_mat.*(ICC_mat>.4); % according to K. O. McGraw, S. P (1996).
        ICC_3D(:,:,t) = ICC_mat>.4;
...
if flags.TimeScale2==1
    ...
    for t=1:n_wsize
        ...
        ICC_avg(:,t) = sum(ICC_mat)./(configs.Nparc-1);
        Max_ICC_nodal(:,t) = max(ICC_mat,[],2);     
        Good_ICC = ICC_mat.*(ICC_mat>.4); % according to wiki
        ICC_3D(:,:,t) = ICC_mat>.4;
